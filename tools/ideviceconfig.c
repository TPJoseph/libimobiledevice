...
#include <libimobiledevice/mobileconfig.h>
...
static void print_usage(int argc, char **argv)
{
	...
	printf("  install FILE\tInstalls the configuration profile specified by FILE.\n");
	printf("              \tA valid .mobileconfig file is expected.\n");
    printf("  erase\t\tAttempt to erase the device via MCInstall.\n");
	printf("  list\t\tGet a list of all configuration profiles on the device.\n");
	printf("  copy PATH\tRetrieves all configuration profiles from the device and\n");
	printf("           \tstores them into the existing directory specified by PATH.\n");
	printf("           \tThe files will be stored as UUID.mobileconfig\n");
	printf("  copy UUID PATH  Retrieves the configuration profile identified by UUID\n");
	printf("           \tfrom the device and stores it into the existing directory\n");
	printf("           \tspecified by PATH. The file will be stored as UUID.mobileconfig.\n");
	printf("  remove UUID\tRemoves the configuration profile identified by UUID.\n");
	printf("  remove-all\tRemoves all installed configuration profiles.\n");
	printf("  dump FILE\tPrints detailed information about the configuration profile\n");
...
int main(int argc, char *argv[])
{
		...
		if (pl) {
			if (output_xml) {
				...
				if (pl && (plist_get_node_type(pl) == PLIST_DICT)) {
					plist_write_to_stream(pl, stdout, 0,0);
				} else {
	...
	if (LOCKDOWN_E_SUCCESS != (ldret = lockdownd_client_new_with_handshake(device, &client, "ideviceconfig"))) {
	...
	mobileconfig_client_t mis = NULL;
	if (mobileconfig_client_new(device, service, &mis) != MOBILECONFIG_E_SUCCESS) {
	...
	switch (op) {
		case OP_INSTALL:
		{
			...
			if (mobileconfig_install(mis, pdata) == MOBILECONFIG_E_SUCCESS) {
				printf("Profile '%s' installed successfully.\n", param);
			} else {
				int sc = mobileconfig_get_status_code(mis);
        ...
        case OP_ERASE:
        {
            if(mobileconfig_erase(mis) == MOBILECONFIG_E_SUCCESS) {
                printf("Device erased successfully\n");
            } else {
                int sc = mobileconfig_get_status_code(mis);
                fprintf(stderr, "Could not erase device, status code: 0x%x\n", sc);
            }
        }
            break;
		case OP_LIST:
		case OP_COPY:
		{
			...
			mobileconfig_error_t merr;
			if (product_version < 0x090300) {
				merr = mobileconfig_copy(mis, &profiles, 0);
			} else {
				merr = mobileconfig_copy_all(mis, &profiles, 0);
			}
			if (merr == MOBILECONFIG_E_SUCCESS) {
				int found_match = 0;
				...
				for (j = 0; !found_match && j < num_profiles; j++) {
					...
					if (profile && (plist_get_node_type(profile) == PLIST_STRING)) {
						plist_get_string_val(profile, &p_name);
						plist_get_string_val(profile, &p_uuid);
					...
					if (op == OP_COPY) {
						char pfname[512];
						if (p_uuid) {
							sprintf(pfname, "%s/%s.mobileconfig", (param2) ? param2 : param, p_uuid);
						} else {
							sprintf(pfname, "%s/profile%d.mobileconfig", (param2) ? param2 : param, j);
						}
		...
		case OP_REMOVE:
			if (param) {
				...
				mobileconfig_error_t merr;
				if (product_version < 0x090300) {
					merr = mobileconfig_copy(mis, &profiles, 1);
				} else {
					merr = mobileconfig_copy_all(mis, &profiles, 1);
				}
				if (merr == MOBILECONFIG_E_SUCCESS) {
					char* p_uuid = NULL;
					uint64_t p_version = -1LL;
					node = plist_dict_get_item(profiles, param);
					if (node && (plist_get_node_type(node) == PLIST_DICT)) {
						uuid_node = plist_dict_get_item(node, "PayloadUUID");
						if (uuid_node && (plist_get_node_type(uuid_node) == PLIST_STRING)) {
							plist_get_string_val(uuid_node, &p_uuid);
						} else {
							fprintf(stderr, "Unexpected PLIST_TYPE from PayloadUUID\n");
						}

						version_node = plist_dict_get_item(node, "PayloadVersion");
						if (version_node && (plist_get_node_type(version_node) == PLIST_UINT)) {
							plist_get_uint_val(version_node, &p_version);
						} else {
							fprintf(stderr, "Unexpected PLIST_TYPE from PayloadVersion\n");
						}
					} else {
						plist_free(node);
						fprintf(stderr, "Unexpected PLIST_TYPE from profiles\n");
						res = -1;
					}

					if (mobileconfig_remove(mis, param, p_uuid, p_version) == MOBILECONFIG_E_SUCCESS) {
				...
				// if (mobileconfig_remove(mis, param) == MOBILECONFIG_E_SUCCESS) {
				// 	printf("Profile '%s' removed.\n", param);
				// } else {
				// 	int sc = mobileconfig_get_status_code(mis);
				// 	fprintf(stderr, "Could not remove profile '%s', status code 0x%x\n", param, sc);
